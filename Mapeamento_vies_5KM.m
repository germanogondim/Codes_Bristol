...
for mes=1:12
    ...
    CLIM_OBS5K(mes,:)=mean(PM_OBS5KM(xd,:),1);
end
%% Bias correction factor
for iii=1:12
    xd=find(datas_m(:,2)==iii);
    for ii=1:size(CLIM_S5KM,1)
        FC_5KM{ii,1}(iii,:)=CLIM_OBS5K(iii,:)./CLIM_S5KM{ii,1}(iii,:);
    end
end
%% Smothing
for iii=1:size(FC_5KM,1)
    for ii=1:size(FC_5KM{1,1},1)
        MB=zeros(size(LAT))/0;
        MBS=zeros(size(LAT))/0;
        MB(in_5k)=FC_5KM{iii,1}(ii,:);
        [x,y]=find(MB>=0);
        for i=1:length(x)
            xx=[x(i)-1:x(i)+1];xx=xx'*ones(1,length(xx)); % 3X3 matrix moving box
            yy=[y(i)-1:y(i)+1];yy=ones(length(yy),1)*yy;  % 3X3 matrix moving box
            xx(xx<=0)=NaN;xx(xx>size(LAT,1))=NaN;         % NaN for values outside the boundaries of the Grid
            yy(yy>size(LAT,2))=NaN;yy(yy<=0)=NaN;         % NaN for values outside the boundaries of the Grid
            xx=reshape(xx,[9 1]);
            yy=reshape(yy,[9 1]);
            PP=((size(LAT,1))*(yy-1))+xx;                 % Moving box position
            mb=zeros(size(PP))/0;
...
save('Fator_Correçao_Final','FCS_5KM')
%% PLOT
...
f = gcf;
exportgraphics(f,['FC_5KM.png'],'Resolution',300)
%% BOXPLOT
nplotx=4;
nploty=3;
CC=1:(nplotx*nploty);
...
f = gcf;
exportgraphics(f,['Boxplot_FC_5KM.png'],'Resolution',300)