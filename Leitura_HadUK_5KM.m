...
P_OBS5KM=zeros(size(datas_obs,1),length(in_5k));
...
for ano=anoi:anof
    for mes=1:12
        ...
        pr=ncread(['C:\Users\kl23221\OneDrive - University of Bristol\Data\HadUK_5km\rainfall_hadukgrid_uk_5km_day_' di '-' df '.nc'],'rainfall');
        pr=rot90(pr);
        pr=pr(14:257,:,:); % Cut to make the grid be exactly same size as UKCP
        clear pp
        for ii=1:size(pr,3)
            p=pr(:,:,ii);
            p=p(in_5k);
...
save('HadUK_Grid','P_OBS5KM','PM_OBS5KM','datas_obs','-v7.3')
